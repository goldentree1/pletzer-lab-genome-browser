...
print_passed_test(){
    ...
    if [ "$ARG_FAILED" == false ]; then
        ...
        echo -e "\e[31mTEST FAILED\e[0m (scripts/build.sh ./test/tmp)"
    fi
}
...
cmd_stdout=$(scripts/build.sh -y ./test/NOT/A/DIR | tee /dev/tty)
clean_stdout=$(echo "$cmd_stdout" | sed 's/\x1B\[[0-9;]*[mK]//g')

# expected output:
read -r -d '' expected <<'EOF'
Directory './test/NOT/A/DIR' does not exist.
...
if check_output "$clean_stdout" "$expected"; then
    print_passed_test "Invalid directory throws error:\n\tscripts/build.sh ./test/tmp" false
else
    print_passed_test "Invalid directory throws error:\n\tscripts/build.sh ./test/tmp" true
fi

# ===============================================================
# TEST #2 -- Valid and invalid test files from 'test/test_files/'
...
rm -rf ./test/tmp
cp -r ./test/test_files/mix ./test/tmp

# run cmd and capture output
cmd_stdout=$(scripts/build.sh -y ./test/tmp | tee /dev/tty)
clean_stdout=$(echo "$cmd_stdout" | sed 's/\x1B\[[0-9;]*[mK]//g')
...
# Check & print success/failure message
if check_output "$clean_stdout" "$expected"; then
    print_passed_test "Valid and invalid file structures are handled appropriately:\n\tscripts/build.sh ./test/tmp" false
else
    print_passed_test "Valid and invalid file structures are handled appropriately:\n\tscripts/build.sh ./test/tmp" true
fi
...
# setup test files in a tmp dir
rm -rf ./test/tmp
cp -r ./test/test_files/valid ./test/tmp
...
# ===============================================================
# TEST #3 -- Valid test files from 'test/test_files/valid'. This should
...
for file in ./test/tmp/simple-valid/reads/sample1/*.bw; do
