...
To build the website, bioinformatics tools are required to check for errors, and convert the raw 'BAM' files into BigWig files, readable by JBrowse. My script 'scripts/build.sh' handles this and builds the website with Vite. It uses the tools 'samtools', 'bamCoverage', 'bigwigAverage' and 'agat' (these are all installed in the Conda environment).
