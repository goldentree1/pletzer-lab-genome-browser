...
#
# NOTE: this bash code is a real mess. Sadly, changing to having multiple experiments
# (instead of 1 experiment per strain) and info.txt for experiment required major data struct refactoring,
# and I didnt have time to tidy it. If you want to clean it up - the outputted data structure should look like
# types.ts -> JBrowseCustomConfigHybrid.
# Only the below function genome_data_processing_ritual() would require changes.
#
...
genome_data_processing_ritual(){
    ...
    for experiment_dir in "$experiments_dir"/*; do
        ...
        #
        if [[ -f "$experiment_dir/info.txt" ]]; then
            info_json="\"info\": \"experiments/$exp_name/info.txt\","
        fi
        ...
        for condition_dir in "$experiment_dir"/*; do
            ...
            condition_name=$(basename "$condition_dir")
            coverage_arrays+=( "[ $(IFS=,; echo "${bw_files[*]}") ]" )
            coverage_condition_names+=( "\"$condition_name\"" )
        done
        coverage_json="{
          ${info_json}
    ...
    # Evil Javascript hackery to finally build CONFIG!! 😮
...
replace_public_data_rsync() {
    local dir="$1"
    # copy config.json (this tells the website how the data is structured)
    cp "$dir/config.json" "config.json"
    # copy the data to ./public/data (this is served at <PAGE_URL>/data/**)
